...
struct FloatInt
{
  using type = std::conditional_t<
      ...
      std::conditional_t<
          ...
          std::conditional_t<sizeof(FloatType) == 4, std::int32_t,
                             std::conditional_t<sizeof(FloatType) == 8,
                                                std::int64_t, void>>
      >
  >;
 ...
 */
template <typename FloatType>
FloatType invSqrt(FloatType x)
{
  union
  {
    FloatType f;
    typename FloatInt<FloatType>::type i;
  } fi;
  fi.f = x;
