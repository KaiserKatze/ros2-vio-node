...
struct EuRoC
{
  ...
  EuRoC()
  {
    ...
    cameraMatrix0 = (cv::Mat_<double>(3, 3) << fu0, 0.0, cu0, 0.0, fv0, cv0,
                     0.0, 0.0, 1.0);
    distCoeffs0   = (cv::Mat_<double>(4, 1) << k01, k02, p01, p02);
    cameraMatrix1 = (cv::Mat_<double>(3, 3) << fu1, 0.0, cu1, 0.0, fv1, cv1,
                     0.0, 0.0, 1.0);
    distCoeffs1   = (cv::Mat_<double>(4, 1) << k11, k12, p11, p12);
    ...
    // 3. 打印一下确认矩阵是否为空 (调试用)
    if (stereoR.empty() || stereoT.empty())
    {
      std::cerr << "Error: stereoR or stereoT is empty!" << std::endl;
      return;
    }
