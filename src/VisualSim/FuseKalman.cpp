...
struct FuseKalman : public rclcpp::Node
{
  ...
  {
    // 1. 在 IMU 高频帧上驱动线性卡尔曼滤波器执行状态演进 (predict)
    filter_.kf_.predict();
    ...
    CreateFusedPose(now_time.nanoseconds(),
                    Eigen::Vector3d(est_x, est_y, est_z), est_q);
  }
  ...
  void PublishTrajectory()
  {
    ...
    pose_msg_.poses.clear(); // 修复：必须清空以确保 pose_msg 只发布一帧
    pose_msg_.poses.push_back(pose);
    index = (index + 1) % path_msg_.poses.size();
    publisher_pose_est_->publish(pose_msg_); // 修复：添加丢失的发布操作
  }
