...
template <typename value_type> struct Path
{
                ...
                const Room<value_type> &room)
  {
    ...
    Point3 basis_z{(center - body_pos).normalized()};    // Z 轴
    ...
    R_body.col(0) = basis_x;
    R_body.col(1) = basis_y;
    R_body.col(2) = basis_z;

    // if constexpr (false)
    {
      auto center_body{R_body.transpose() * (center - body_pos)};
      ...
      auto center_pixel_left{
          rig.camera_left_.ProjectPoint(center, R_body.transpose(),
      ...
      auto center_pixel_right{
          rig.camera_right_.ProjectPoint(center, R_body.transpose(),
                                         -R_body.transpose() * body_pos),
      };
      std::cerr << std::fixed << std::setprecision(1)
